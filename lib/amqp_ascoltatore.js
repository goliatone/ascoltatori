    ...
    async.series([
      ...
      function(callback) {
        that._queue.subscribe({
          ...
          //If we publish a message with content type header of application/json
          //this blows up.
          function getMessage(data){
            if(message.data && message.data instanceof Buffer) return message.data.toString();
            if(message.toString() === '[object Object]') return JSON.stringify(message);
          }
          that._ascoltatore.publish(topic, getMessage(message));
        });
